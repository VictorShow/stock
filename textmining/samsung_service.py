...
class SamsungService:
    ...
    def extract_hanguel(self):
        print('>>>>> 한글만 추출.')
        tests = self.texts.replace('\n', ' ')
        tokenizer = re.compile(r'[^ ㄱ-힣]')
        self.texts = tokenizer.sub('', tests)
        #print(f'{self.texts[:300]}')
    ...
    def conpound_noun(self):
        print('>>>>> 복합명사는 묶어서 filteing으로 출력\n')
    ...
    def filtering_text_with_stopword(self):
        print('>>> filtering_text_with_stopword')
    ...
    def draw_wordloud(self, payload):
        print('>>> Wordloud')
        filename = payload.context + payload.fname
